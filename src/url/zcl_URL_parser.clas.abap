...
class zcl_url_parser definition public
    inheriting from zcl_url_grammar
    create public.
  ...
  public section.
    ...
    methods format_error importing input type string
                                   offset type i
                                   expected type zcl_URL_grammar=>string_string_tab
                         returning value(result) type string.
endclass.

class zcl_url_parser implementation.
  method constructor.
    super->constructor( ).
    me->input = input.
  ...
  method parse.
    data(parser) = new zcl_url_parser( input = input actions = actions ).
  ...
  method parse3.
    ...
    if expected is initial.
      failure = offset.
      append value stringtab( ( `URL` ) ( `<EOF>` ) ) to expected.
    endif.
